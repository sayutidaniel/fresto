...
export function getGoogleStaticMapUrl({ height, marker, width }) {
  const params = {
    key: GOOGLE_MAP_API_KEY,
    markers: Object.keys(marker).map((key) => key !== 'location' ? `${key}:${marker[key]}` : marker[key]).join('%7C'),
    size: `${width}x${height}`,
  };

  return `https://maps.googleapis.com/maps/api/staticmap?${Object.keys(params).map((key) => `${key}=${params[key]}`).join('&')}`;
}

export function createMarkerOverlay(google, coordinates) {
  class MarkerOverlay extends google.maps.OverlayView {
    ...
    onAdd() {
      ...
      this.tooltips = this.coordinates.map((coordinate, index) => {
        ...
        tooltip.className = classNames(styles.tooltip, Bootstrap.tooltip, Bootstrap['top'], Bootstrap['in']);
        tooltipArrow.className = classNames(styles.tooltipArrow, Bootstrap['tooltip-arrow']);
        tooltipInner.className = classNames(styles.tooltipInner, Bootstrap['tooltip-inner']);
  ...
  return new MarkerOverlay(coordinates);
}
